    ...
    <div class="container-fluid">
        <div class="row p-5">
            ...
            <div class="col-md-12 mb-4">
                <form action="{{ route('galleries.store') }}" method="POST" enctype="multipart/form-data" class="dropzone" id="gallery-dropzone">
                    @csrf
                </form>
            </div>
            ...
            <div class="col-md-12">
                <form action="{{ route('galleries.destroy') }}" method="POST" id="delete-form">
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            @foreach($galleries as $gallery)
                                <tr>
                                    <td>
                                        <input type="checkbox" id="image{{ $gallery->id }}" name="selected_images[]" value="{{ $gallery->id }}">
                                    </td>
                                    <td>
                                        <label for="image{{ $gallery->id }}">
                                            <img src="{{ asset($gallery->image_url) }}" alt="Fotoğraf" class="img-thumbnail" style="width: 150px;">
                                        </label>
                                    </td>
                                </tr>
                            @endforeach
                    ...
                    <button type="button" class="btn btn-danger mt-2" id="delete-button">Seçili Fotoğrafları Sil</button>
                </form>
            </div>
        </div>
    </div>
